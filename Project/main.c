...
extern int _asm_test_loops(int durchlauefe);
...
int main(int argc, char **argv) {
	printf("\nMain hello\n\n");
	uint asmTest = _asmAdd(5, 8);
	printf("5+8=%i\n", asmTest);
	asmTest = _asm_test_program(5);
	printf("Die Summe der Inhalte der %i Speicherplaetze auf dem Heap war %u\n", 5, asmTest);
	...
	if (argc > 1) {
		...
		if(loop_e == 1){
			int durchlaeufe = _asm_test_loops(loop_e_N);
			printf("Es wurde %i mal eine leere Schleife durchlaufen %i\n", loop_e_N, durchlaeufe);
		}
//T4.2.4
		if(argc > 3){
